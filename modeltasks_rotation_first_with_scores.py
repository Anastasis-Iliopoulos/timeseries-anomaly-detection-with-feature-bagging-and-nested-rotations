...
def ensemble_conv_ae(df, ds, df_name, task_name, K=4, fraction=0.65):
    ...
    df_final = utils.get_actual_scores_for_windows(residuals_conv_ae, df, X_conv_ae, 60, UCL_conv_ae, f"anomaly_by_conv_ae_rotation_task_{task_name}_score", f"anomaly_by_conv_ae_rotation_task_{task_name}_ucl")
...
def ensemble_lstm(df, ds, df_name, task_name, K=4, fraction=0.65):
    ...
    predictions_lstm = anomalyutils.get_lstm_predicts(lstm_model, X_lstm)
    residuals_lstm = anomalyutils.get_lstm_residuals(y_lstm, predictions_lstm)
    prediction_labels_lstm = pd.DataFrame(pd.Series(residuals_lstm.values, index=df[5:].index).fillna(0)).rename(columns={0:f"anomaly_by_lstm_rotation_task_{task_name}_score"})
    df_to_append = pd.DataFrame(pd.Series(0, index=df[:5].index).fillna(0)).rename(columns={0:f"anomaly_by_lstm_rotation_task_{task_name}_score"})
    df_final = pd.concat([df_to_append, prediction_labels_lstm], ignore_index=False)
    df_final[f"anomaly_by_lstm_rotation_task_{task_name}_ucl"] = 3/2*UCL_lstm
    return df_final
...
def ensemble_lstm_ae(df, ds, df_name, task_name, K=4, fraction=0.65):
    ...
    df_final = utils.get_actual_scores_for_windows(residuals_lstm_ae, df, X_lstm_ae, 10, UCL_lstm_ae, f"anomaly_by_lstm_ae_rotation_task_{task_name}_score", f"anomaly_by_lstm_ae_rotation_task_{task_name}_ucl")
    return df_final
...
def ensemble_lstm_vae(df, ds, df_name, task_name, K=4, fraction=0.65):
    ...
    df_final = utils.get_actual_scores_for_windows(residuals_lstm_vae, df, X_lstm_vae, 5, UCL_lstm_vae, f"anomaly_by_lstm_vae_rotation_task_{task_name}_score", f"anomaly_by_lstm_vae_rotation_task_{task_name}_ucl")
    return df_final
