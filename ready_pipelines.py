...
def run_pipeline_conv_ae_for_new_data_1(SAVE_FOLDER, nr_subsets, nr_percentage ,family_descr, task_label, list_of_df, seed):
    set_random(seed)
    for  tmp_file_ind, df in tqdm(enumerate(list_of_df), str(task_label)):
        ...
        tmp_model_obj.pipeline_fit(df[:400])
        final_df = tmp_model_obj.pipeline_transform(df)
    return 1